...
namespace hs071_cs
{
    ...
    public class Program
    {
        public static void Main()
        {
            ...
            Print("\n=== Результат расчётов ===");
            Data optimalPoint = new Data(xIter, yIter, zIter, rNach, RIter, TotalBallCount, holeCount: holesCount, taskClassification: TaskClassification.FixedRadiusTask, type: objectType, Weight: weight, C: lengthBond);
            Show(optimalPoint);
            Print("\nQuality of solution = " + Density(rNach, RIter));

            OutPut.SaveToFileAllIteration(optimalPoint, IpoptIterationData, "IterationFixedRadius", true);
            SaveToFile(optimalPoint, "CoordinateWithHoles"); // запись результата в файл

            // unfixed radius
            RandomizeStartPoint(optimalPoint, out double[] constantRadius);
            using (UnFixedRadius3dAdaptor adaptor = new UnFixedRadius3dAdaptor(optimalPoint, constantRadius))
            {
                double[] xyzUnFixR;
            ...
            Data optionalPointWithUnfixedRadius = new Data(xIter, yIter, zIter, rNach, RIter, ballCount: TotalBallCount, holeCount: holesCount, taskClassification: TaskClassification.VariableRadius, type: objectType, Weight: weight, C: lengthBond);

            OutPut.SaveToFileAllIteration(optionalPointWithUnfixedRadius, IpoptIterationData, "IterationUnfixedRadius", true);
            Print("\n\nUNFIXED_RADIUS");
            Show(optionalPointWithUnfixedRadius);
        ...
        private static void CreateArrayWithObjectType(Data data, out ObjectType[] objectType)
        {
            ...
            for (int i = 0, j = 0; i < data.ball.Length; i++)
            {
                if (i <= j)
                {
                    ...
                    {
                        data.ball[j].ObjectType = objectType[j] = ObjectType.ProhibitionZone;
                    }
                }
                data.ball[i].ObjectType = objectType[i] = ObjectType.SimpleBall;
            }
        }
