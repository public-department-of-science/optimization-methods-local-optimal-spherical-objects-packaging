...
namespace hs071_cs
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            // gen weight
            double[] weight = new double[circlesCount];
            for (int i = 0; i < weight.Length; i++)
            {
                weight[i] = (4.0 / 3.0) * rNach[i] * rNach[i] * rNach[i] * Math.PI;
            }
            weight = weight.OrderBy(w => w).ToArray();
            ...
            SaveToFile(xNach, yNach, zNach, rNach, RNach, "StartPoint", weight);
            Console.WriteLine("=== ================== ===");
            ...
            using (var adaptor = new FixedRadius3dAdaptor(rNach, xNach, yNach, zNach, weight, RNach))
            {
            ...
            Console.WriteLine("Quality of solution = {0}", Unbalance(xIter, yIter, zIter, weight));
        ...
        static void RunTask(FixedRadius3dAdaptor op, double[] xyz, out double[] NewX, out double[] NewY, out double[] NewZ)
        {
            ...
            using (Ipopt problem = new Ipopt(op._n, op._x_L, op._x_U, op._m, op._g_L, op._g_U, op._nele_jac, op._nele_hess, op.eval_f, op.eval_g, op.eval_grad_f, op.eval_jac_g, op.eval_h))
            {
                ...
                problem.AddOption("print_level", 1); // 0 <= value <= 12, default is 5
        ...
        static void SaveToFile(double[] x, double[] y, double[] z, double[] r, double R, string fileName, double[] weight = null)
        {
            try
            {
                ...
                if (weight != null)
                {
                    ...
                    using (StreamWriter sw = new StreamWriter(writePath, true, System.Text.Encoding.Default))
                    {
                        ...
                        for (int i = 0; i < x.Length; i++)
                        {
                            sw.Write(weight[i].ToString().Replace(',', '.') + " ");
                            sw.WriteLine();
                        }
                        sw.Close();
                        Console.WriteLine("All Data has been writen to {0}!", writePath);
                    }
                }
            }
            catch (Exception ex)
        ...
        private static double Unbalance(double[] x, double[] y, double[] z, double[] Weight)
        {
            double Unbalance = 0.0;
            for (int i = 0; i < x.Length; i++)
            {
                Unbalance += Math.Pow(Weight[i] * x[i], 2)
                    + Math.Pow(Weight[i] * y[i], 2)
                    + Math.Pow(Weight[i] * z[i], 2);
            }
            return Unbalance;
        }
    }
