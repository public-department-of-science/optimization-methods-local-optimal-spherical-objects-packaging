...
namespace hs071_cs
{
    class UnFixedRadius3dAdaptor : BaseAdaptor, IDisposable
    {
        ...
        public UnFixedRadius3dAdaptor(Data data)
        {
            ...
            _n = ballsCount * 4 + 1; // количество переменных в векторе
            sortedRadius = new double[ballsCount];
            ...
            * *************************************************************************************/
            ...
            for (int i = 0; i < ballsCount; i++)
            {
                ...
                _x_U[4 * i + 3] = sortedRadius[ballsCount - 1];
            }
            _x_L[_n - 1] = sortedRadius[ballsCount - 1]; // От максимального радиуса
            ...
            elementsInTranspositionPolyhedron = (int)Math.Pow(2, ballsCount);
        ...
        {
            int countOfelements = (int)Math.Pow(2, ballsCount);
            string temp;
        ...
        public override bool eval_jac_g(int n, double[] x, bool new_x, int m, int nele_jac, int[] iRow, int[] jCol, double[] values)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < elementsInJacLinPolyhedron - ballsCount; i++)
                {
                    values[kk++] = 1;
                }
                for (int i = 0; i < ballsCount; i++)
