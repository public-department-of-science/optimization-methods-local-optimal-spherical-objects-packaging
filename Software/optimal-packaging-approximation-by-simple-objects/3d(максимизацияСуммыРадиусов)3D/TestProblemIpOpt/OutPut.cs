    ...
    public static class OutPut
    {
        ...
        public static void ShowData(Data data)
        {
            ...
            for (int i = 0; i < cicleCount; i++)
            {
                ...
                WriteLine("Z", iterCount++, data.ball[i].Z);
                WriteLine("R", iterCount++, data.ball[i].R);
                WriteLine("_____________");
            }
            Console.WriteLine($"External_Ball_Radius = {data.R}");
        }
        public static void SaveToFileAllIteration(Data data, List<double[]> allIteration, string fileName)
        {
            try
            {
