...
namespace hs071_cs
{
    class FixedRadius3dAdaptor : BaseAdaptor, IDisposable
    {
        ...
        public double Fine = 10; // траф функции

        public double SummedWeight { get; }
        private double[,] C { get; set; }

        public FixedRadius3dAdaptor(Data data)
        {
            countCircles = data.circleCount; // задаём количетво кругов
            _n = countCircles * 3; // количество переменных в векторе
            ...
            * *************************************************************************************/
            ...
            Weight = new double[countCircles];
        ...
        {
            // R -> min
            obj_value = -Fine * (Math.Pow(SummedWeight * SummedWeight * SummX(x), 2.0)) +
                Fine * (Math.Pow(SummedWeight * SummedWeight * SummY(x), 2.0)) +
                Fine * (Math.Pow(SummedWeight * SummedWeight * SummZ(x), 2.0)) + Fine * C_Multiply_by_Length(x);
            return true;
        }
        public override bool eval_grad_f(int n, double[] x, bool new_x, double[] grad_f)
        {
            ArrayOfDerivatives(x, out double[] Xderivative, out double[] Yderivative, out double[] Zderivative);
            for (int i = 0; i < countCircles; i++)
            {
                grad_f[3 * i] = Xderivative[i] - 2 * Fine * x[i] * Weight[i]; ;
                grad_f[3 * i + 1] = Yderivative[i] - 2 * Fine * x[i] * Weight[i]; ;
                grad_f[3 * i + 2] = Zderivative[i] - 2 * Fine * x[i] * Weight[i]; ;
            }
            return true;
        }
