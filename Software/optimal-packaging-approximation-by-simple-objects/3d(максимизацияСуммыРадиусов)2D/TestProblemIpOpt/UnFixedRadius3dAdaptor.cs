...
namespace hs071_cs
{
    class UnFixedRadius3dAdaptor : BaseAdaptor, IDisposable
    {
            ...
            * *************************************************************************************/
            ...
            for (int i = 0; i < countCircles; i++)
            {
                if (data.ball[i].ObjectType == (ObjectType)2)
                {
                    ...
                    _x_L[4 * i + 2] = data.ball[i].Z = 0;
                    _x_U[4 * i + 2] = data.ball[i].Z = 0;
                ...
                else
                {
                    ...
                    _x_L[4 * i + 2] = 0;// -K1 * DiametrSum(sortedRadius) + sortedRadius[countCoordinate];
                    _x_U[4 * i + 2] = 0;// K1 * DiametrSum(sortedRadius) - sortedRadius[countCoordinate];

                    _x_L[4 * i + 3] = 0;
                    _x_U[4 * i + 3] = K1 * DiametrSum(sortedRadius) - sortedRadius[countCoordinate];
                }
        ...
        {
            // R -> min
            obj_value = K2 * C_Multiply_by_Length(x) - Math.PI * SumRadiusPowered(x, 2);//+ K1 * x[_n - 1];
        ...
        public override bool eval_grad_f(int n, double[] x, bool new_x, double[] grad_f)
        {
            ...
            Parallel.For(0, countCircles, i =>
            {
                ...
                grad_f[4 * i + 3] = K2 * Rderivative[i] - 2 * Math.PI * SumRadiusPowered(x, 1);
            });
            #endregion
            grad_f[_n - 1] = 0;// K1 * 1;
            return true;
